...
#是获取城市的页面下的所有职位列表URL
def get_item_urls(city_url,page):
    r = requests.get(city_url+str(page),headers=headers)
    if r.status_code == 200:
        html = etree.HTML(r.text)
...
def pose_urls_into_redis(data_dase_name,pose_urls_list):
    a = data_storagy.redis_data(data_dase_name)
    for i in pose_urls_list:
        a(i)
if __name__ == '__main__':
    pose_urls = []
    for i in range(1,20):
        urls = get_item_urls(city_url["深圳"],i)
        pose_urls.extend(urls)

    pose_urls_into_redis("SZ_POSE_URLS",pose_urls)


    pose_urls = []
    for i in range(1,20):
        urls = get_item_urls(city_url["北京"],i)
        pose_urls.extend(urls)

    pose_urls_into_redis("BJ_POSE_URLS",pose_urls)


    pose_urls = []
    for i in range(1,20):
        urls = get_item_urls(city_url["上海"],i)
        pose_urls.extend(urls)

    pose_urls_into_redis("SH_POSE_URLS",pose_urls)
        # print(len(pose_urls))
